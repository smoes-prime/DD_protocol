#!/usr/bin/env bash
# run_hasten_equivalent_simulation.sh
# End-to-end driver for the Deep Docking simulation pipeline configured to match
# HASTEN's 10% dataset sampling framework.
#
# Usage:
...
#   ./utilities/run_hasten_equivalent_simulation.sh
...
# ==============================================================================
# HASTEN Equivalence Configuration
# If the total dataset is ~21,000,000 molecules, 10% is 2,100,000.
# We set the training set to 2,100,000 to match HASTEN's learning fraction.
# We also reduce the validation/test sets to 500k to keep the iteration sampling fast.
# ==============================================================================
TRAIN_SIZE="${TRAIN_SIZE:-2100000}"
VAL_SIZE="${VAL_SIZE:-500000}"
TOTAL_SAMPLING=$((TRAIN_SIZE + VAL_SIZE + VAL_SIZE))

echo "=== Deep Docking (HASTEN 10% Equivalent) ==="
echo "Project: $PROJECT_DIR"
echo "CPUs: $N_CPU"
echo "Data Root: $DD_DATA_ROOT"
echo "Master Scores: $MASTER_SCORES_PATH"
echo "Train Size: $TRAIN_SIZE (10% of 21M)"
echo "Val/Test Size: $VAL_SIZE"
echo "============================================="
...
python scripts_1/molecular_file_count_updated.py \
  -pt "$PROTEIN" -it 1 -cdd "$DD_DATA_ROOT/library_morgan" -t_pos "$N_CPU" -t_samp "$TOTAL_SAMPLING"
...
ITER=1 VAL_SIZE="$VAL_SIZE" RECALL="$RECALL" ./utilities/finish_iteration_phase5.sh
...
START_ITER=2 END_ITER="$TOTAL_ITERATIONS" TRAIN_SIZE="$TRAIN_SIZE" VAL_SIZE="$VAL_SIZE" TOTAL_SAMPLING="$TOTAL_SAMPLING" RECALL="$RECALL" ./utilities/run_dd_simulation_pipeline.sh
